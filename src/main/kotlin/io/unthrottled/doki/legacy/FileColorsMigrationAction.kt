...
object FileColorsMigrationAction {

  private fun isSet(project: Project?): Boolean {
  ...
  fun setDefaultFileScopes(project: Project?) {
    if (project != null && isSet(project)) {
  ...
  private fun getScopes(): List<Pair<String, String>> =
    ThemeManager.instance.currentTheme
      .map { selectedTheme ->
        listOf(
          Pair(NonProjectFilesScope.NAME, "Yellow"),
          Pair(TestsScope.NAME, "Green"),
          Pair("Local Unit Tests", "Green"),
          //  dis android bundle -> String message = AndroidBundle.message("android.test.run.configuration.type.name");
          Pair("Android Instrumented Tests", "Green")
        )
